...
def _load_vtd00_sources(path_or_dir: str) -> gpd.GeoDataFrame:
    if os.path.isdir(path_or_dir):
        zips = sorted(glob.glob(os.path.join(path_or_dir, "tl_2010_45*_vtd00.zip")))
        if not zips:
            raise SystemExit(f"No VTD00 county zips found in {path_or_dir}")
        parts = [gpd.read_file(p) for p in zips]
        return gpd.GeoDataFrame(pd.concat(parts, ignore_index=True), crs=parts[0].crs)
    if os.path.isfile(path_or_dir):
        return gpd.read_file(path_or_dir)
    raise SystemExit(f"Missing source path: {path_or_dir}")


def main() -> None:
    ap = argparse.ArgumentParser(description="Build VTD00 -> VTD20 overlap crosswalk CSV.")
    ap.add_argument("--base", default=os.path.join(os.path.dirname(__file__), ".."), help="Repo base path")
    ap.add_argument(
        "--source",
        default=os.path.join("Data", "vtd00_counties"),
        help="Source VTD00 path: dir of county zips or a single VTD00 file",
    )
    ...
    source = _load_vtd00_sources(source_path)
    target = gpd.read_file(target_path)

    _require_columns(source, ["COUNTYFP00", "NAME00", "geometry"], "Source")
    ...
    source["county_fips"] = source["COUNTYFP00"].astype(str).str.zfill(3)
    source["source_name"] = source["NAME00"].astype(str).str.strip()
